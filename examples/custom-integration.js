...
exports.customIntegrationExample = customIntegrationExample;
...
async function customIntegrationExample() {
    ...
    webhookManager.onWebhookEvent("data.updated", async (payload, context) => {
        ...
        console.log(`📊 Data updated: ${updateData.entity_type}#${updateData.entity_id}`);
    ...
    webhookManager.onWebhookEvent("system.alert", async (payload, context) => {
        ...
        console.log(`🚨 System alert: ${alertData.alert_type} - ${alertData.message}`);
    ...
    try {
        ...
        const tokenResult = await integration.completeOAuth(mockCode, mockState, context);
        if (tokenResult.success) {
            console.log("✅ OAuth completed!");
            console.log("👤 User:", tokenResult.data?.userInfo);
            console.log("🔑 Access Token:", tokenResult.data?.tokens.accessToken.substring(0, 10) + "...");
        }
        // 3. Simulate various webhook events
        console.log("\n🌊 Simulating webhook events...");
        // User creation event
        await simulateCustomWebhook(integration, "user.created", {
            user: {
                id: "user_789",
        ...
        // Data update event
        await simulateCustomWebhook(integration, "data.updated", {
            entity_type: "project",
            entity_id: "proj_123",
            changes: {
        ...
        // System alert event
        await simulateCustomWebhook(integration, "system.alert", {
            alert_id: "alert_999",
...
async function simulateCustomWebhook(integration, event, data, context) {
...
async function sendWelcomeEmail(user) {
    console.log(`📧 Sending welcome email to ${user.email} (mock implementation)`);
    // In real app: await emailService.send('welcome', user);
}
async function syncData(updateData) {
    console.log(`🔄 Syncing ${updateData.entity_type} data (mock implementation)`);
