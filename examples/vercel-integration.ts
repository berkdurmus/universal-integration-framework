...
import { VercelOAuth } from "../src/oauth";
...
async function vercelDeploymentMonitor() {
  ...
  try {
    ...
    const vercelProvider = integration.getOAuthProvider() as VercelOAuth;
    ...
    if (vercelProvider) {
      ...
      try {
        ...
        for (const project of projects.slice(0, 5)) {
          console.log(
            `  - ${project.name} (${project.framework || "Unknown"})`
          );
        }
      } catch (error) {
      ...
      try {
        ...
        for (const deployment of deployments.slice(0, 3)) {
          console.log(
            `  - ${deployment.name}: ${deployment.state} (${deployment.url})`
          );
        }
      } catch (error) {
...
async function simulateVercelWebhook(
